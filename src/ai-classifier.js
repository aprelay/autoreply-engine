...
export async function generateReply(email, account, tenantId = 1) {
  ...
  if (isDomainResearchEnabled(tenantId)) {
    research = await getDomainResearchForEmail(email, tenantId, account);
  }
...
5. Acknowledge what they said specifically (show you read their email)${research && research.summary ? `\n5b. Using the COMPANY RESEARCH, name the SPECIFIC service ${research.companyName || 'their company'} offers that you are interested in, and express that you'd like to engage them for it (you are the interested client). Reference their company by its REAL name only — never a slogan.` : `\n5b. You have NO verified information about what the sender's company does. Do NOT name, guess, or invent any specific service, industry, product, or company name. Keep it general: say you have a business requirement / project you would like to discuss and want to see if there is a fit. Do NOT say you want to "engage" or "hire" any specific company. NEVER refer to your own company as a vendor to engage — your company appears ONLY in the sign-off, never in the body of the message.`}
